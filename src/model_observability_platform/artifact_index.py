...
def render_artifact_index(root: str | Path, *, title: str, description: str, dashboard: str) -> Path:
    ...
    cards = [
        ...
        ("Alert Routing Remediation", "alert_routing_remediation_plan.json", "Alertmanager grouping, inhibition, escalation, Argo Rollouts notifications, guarded remediation, and column-lineage impact."),
        ...
        ("AdminAccess Diagnostics", "admin_access_diagnostics_plan.json", "Kubernetes v1.36 DRA AdminAccess diagnostics for drift checks, incident linkage, evidence quality, and rollout-freeze guardrails."),
        ("In-Place Resize", "inplace_resize_plan.json", "Kubernetes in-place Pod resize, pod-level resource resizing, incident-safe monitor bursts, VPA InPlaceOrRecreate, and resize-status alerts."),
        ...
        ("Stateful Orchestration", "airflow_stateful_orchestration_plan.json", "Airflow 3.3 task/asset state stores, bounded incident rollup and diagnostic fanout, retry policies, and real SDK validation."),
        ...
        ("Multi-Team Readiness", "multi_team_readiness_plan.json", "Airflow multi-team preview readiness for observability-owned DAG Bundles, pools, triggerers, secrets, executors, and asset filtering."),
        ...
        ("Flavor Fungibility", "flavor_fungibility_plan.json", "Kueue ResourceFlavor fallback, TryNextFlavor policies, explicit borrowing/preemption preference, and observability pool trade-offs."),
        ("Pending Workload Visibility", "pending_workload_visibility_plan.json", "Kueue VisibilityOnDemand, raw pendingworkloads API paths, incident queue triage, and admission-wait alerts."),
        ...
        ("Workload-Aware Scheduling", "workload_aware_scheduling_plan.json", "Kubernetes v1.36 Workload/PodGroup readiness for incident root-cause fanout, drift backlog diagnostics, rollout-freeze smoke, topology constraints, DRA sharing, and workload-aware preemption."),
        ("Runtime Security", "runtime_security_plan.json", "Kubernetes v1.36 user namespaces and fine-grained kubelet authorization for incident routers, telemetry readers, and rollout-freeze smoke."),
        ("Control Plane Diagnostics", "control_plane_diagnostics_plan.json", "Kubernetes v1.36 controller staleness, /statusz, /flagz, PSI, and native-histogram readiness for incident automation."),
